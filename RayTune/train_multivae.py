...
parser.add_argument('--save', type=str, default='vae_n2000_e200',
                    help='path to save the final model')
parser.add_argument('--vae', type=str, default=True,
                    help='select VAE or DAE')
parser.add_argument('--n', type=str, default=2000,
...
def train(model, criterion, optimizer, anneal_cap, is_VAE = False):
...
def evaluate(model, criterion, data_tr, data_te, anneal_cap, is_VAE=False):
...
def train_multivae(config, checkpoint_dir=None):
    ###############################################################################
    # Build the model
    ###############################################################################
    model = MultiVAE(config["p_dims_0"], config["p_dims_1"], n_items)
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda:0"
    model.to(device)
    ...
    best_n100 = -np.inf
    anneal_cap = config.get("anneal_cap", 0.2)
    for epoch in range(1, args.epochs + 1):
        epoch_start_time = time.time()
        train(model, criterion, optimizer, anneal_cap, is_VAE=args.vae)
        val_loss, n100, r20, r50 = evaluate(model, criterion, vad_data_tr, vad_data_te, anneal_cap, is_VAE=args.vae)
        print('-' * 89)
